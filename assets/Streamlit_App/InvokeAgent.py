...
def askQuestion(question, url,user, endSession=False):
    myobj = {
        "inputText": question,   
        "enableTrace": True,
        "endSession": endSession,
        # Uncomment the following code for Authentication flow
        # "sessionState": { 
        #   "sessionAttributes": { 
        #      "agentName" : user 
        #   }
...
def lambda_handler(event, context):
    ...
    agentId = "agentId" #INPUT YOUR AGENT ID HERE
    agentAliasId = "agentAliasId" # Hits draft alias, set to a specific alias id for a deployed version
    ...
    try: 
        response, trace_data = askQuestion(question, url, user, endSession)
        return {
            "status_code": 200,
